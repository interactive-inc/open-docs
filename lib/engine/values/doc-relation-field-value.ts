import { zDocRelationField } from "@/lib/models"
import type { DocRelationField } from "@/lib/types"
...
export class DocRelationFieldValue {
   ...
   */
  get relationPath() {
    return this.value.relationPath
  }
  ...
  get targetDirectoryName() {
    const parts = this.value.relationPath.split("/")
  ...
  getFullPath(basePath: string): string {
    if (this.value.relationPath.startsWith("/")) {
      return this.value.relationPath
    }
    return `${basePath}/${this.value.relationPath}`
  }

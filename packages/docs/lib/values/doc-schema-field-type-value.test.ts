...
test("配列型を正しく判定できる", () => {
  ...
  const multiBoolean = new DocSchemaFieldTypeValue("multi-boolean")
  expect(multiBoolean.isArray).toBe(true)
...
test("基本型を正しく取得できる", () => {
  ...
  expect(new DocSchemaFieldTypeValue("multi-boolean").baseType).toBe("boolean")

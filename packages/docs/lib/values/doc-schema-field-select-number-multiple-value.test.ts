...
test("複数選択数値型フィールドの基本プロパティを取得できる", () => {
  const field = new DocSchemaFieldSelectNumberMultipleValue("sizes", {
...
test("オプショナルなプロパティがundefinedでも動作する", () => {
  const field = new DocSchemaFieldSelectNumberMultipleValue("values", {
    type: "multi-select-number",
    required: true,
    options: [10, 20, 30],
    title: null,
...
test("JSON形式に変換できる", () => {
  const field = new DocSchemaFieldSelectNumberMultipleValue("percentages", {
    type: "multi-select-number",
    required: false,
    title: "割合",
    description: null,

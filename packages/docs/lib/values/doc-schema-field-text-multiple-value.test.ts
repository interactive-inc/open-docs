...
test("複数テキスト型フィールドの基本プロパティを取得できる", () => {
  const field = new DocSchemaFieldTextMultipleValue("tags", {
    type: "multi-text",
    required: true,
    title: "タグ",
...
test("JSON形式に変換できる", () => {
  const field = new DocSchemaFieldTextMultipleValue("authors", {
    type: "multi-text",
    required: false,
    title: "著者リスト",
    default: ["田中", "鈴木"],
...
test("インスタンスが不変である", () => {
  ...
  const field = new DocSchemaFieldTextMultipleValue("test", fieldData)
  ...
  expect(() => {
    ;(field.default as any).push("item3")
  }).toThrow()
})
